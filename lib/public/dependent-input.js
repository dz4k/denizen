export class DependentInput extends HTMLElement {
  ...
  update = () => {
    ...
    const selectedValue = input.dataset.controls ?? input.selectedOptions[0].dataset.controls
    const values = getValues(input)
    console.log({ values, selectedValue })
    values.forEach(value => {
      const el = document.getElementById(value)
      if (!el) {
        console.warn('<dependent-input>: nonexistent element referenced:', value)
        return
      }
...
const getValues = el => Array.from(getOptions(el), opt => opt.dataset.controls)
const getOptions = el => {
