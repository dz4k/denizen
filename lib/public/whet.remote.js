 ...
 */
const runJSUri = (args) =>
	new Function('args', 'with(args){return #{args.action.pathname}}')
...
const doFetch = async (context) => {
	...
	const event = dispatchExtendable(element, ':will-fetch', context)
	await event.waited()
...
const swap = (style, context, content) => {
	...
	const selector = (() => {
		let selector = context.action.hash
		if (context.action.hash.startsWith(selectorPrefix)) {
			selector = context.action.hash.slice(selectorPrefix.length)
		}
